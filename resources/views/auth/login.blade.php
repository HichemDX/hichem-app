...
<div class=" w-[480px] h-[643px] shadow-2xl rounded-r-lg ">
      ...
      <main class="py-4">
          ...
          <center>
            @csrf
                <div>
                <input id="email" type="email" class="@error('email') is-invalid @enderror w-[343px] h-[54px] rounded-[15px] shadow-2xl text-xl p-4" name="email" value="{{ old('email') }}" required autocomplete="email" autofocus>
                @error('email')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
